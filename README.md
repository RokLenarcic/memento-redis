...
This creates an infinite cache for function `my-function` in redis with Carmine connection {} (localhost).
...
The latter two will be queried for connection map each time its used.
...
Cache can have a name, specified via `:memento.redis/name` cache conf property. It defaults to `""`, which might can be
ok for many caches, but if use multiple caches to cache the same function (e.g. by swapping cache using 
...
**The most important setting when using this cache.** As all Memento caches, a single Memento Redis cache potentially contains entries for
multiple functions. When stored in Redis, this structure is represented by flat string keys. **It is important how these keys are kept from colliding.**
Sometimes you might want the collisions, in which case the same entries will be returned from more than 1 function. 

Keys are derived from triplets:
- Memento Redis cache name (set by :memento.redis/name cache configuration property)
- Memento Segment (the cached function)
- argument list of the function call

The main thing to configure is how Memento segment part of this triplet is handled. By default,
the segment ID is used as part of the Redis key to disambiguate entries from different MountPoints (cached functions).

Memento itself enables you to provide ID when you bind a function cache using `:memento.core/id` setting:
...
The ID can be any type. If the ID is not specified, then it defaults:
- String qualified name of Var if `memo` is called on a Var
- Java function object if you call `memo` directly on a function 

It is this second scenario that is problematic, and Memento Redis need your input about to name
these anonymous functions in the entries' Redis key.

### 1. The :stringify option
...
This will any sort of problems with entries from different anonymous functions colliding with each other, but 
...
You can fix this by providing `:memento.core/id` to anonymous functions as described above or memoize Vars.
...
### 3. Write your own KeysGenerator
...
Useful when, for one reason or another, you've got cache related keys stuck in Redis, and you
